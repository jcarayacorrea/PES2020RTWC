...
def final_round(request: HttpRequest) -> HttpResponse:
    ...
    context = _service().get_round_context('final', GROUP_KEYS[0:5], team_size=5, group_range=GROUP_RANGE)
...
def third_round(request: HttpRequest) -> HttpResponse:
    ...
    context = _service().get_round_context('third', GROUP_KEYS[0:5], team_size=4, group_range=GROUP_RANGE[0:4])
...
def second_round(request: HttpRequest) -> HttpResponse:
    ...
    context = _service().get_round_context('second', GROUP_KEYS[0:5], team_size=4, group_range=GROUP_RANGE[0:4])
...
def first_round(request: HttpRequest) -> HttpResponse:
    ...
    context = _service().get_round_context('first', GROUP_KEYS[0:3], team_size=4, group_range=GROUP_RANGE[0:4])
...
def first_round_button(request: HttpRequest) -> HttpResponse:
    ...
    if request.method == 'GET':
        _service().perform_draw('first', pools_count=4, teams_per_pool=3, home_away=False)
...
def second_round_button(request: HttpRequest) -> HttpResponse:
    ...
    if request.method == 'GET':
        _service().perform_draw('second', pools_count=4, teams_per_pool=5, home_away=True)
...
def third_round_button(request: HttpRequest) -> HttpResponse:
    ...
    if request.method == 'GET':
        _service().perform_draw('third', pools_count=4, teams_per_pool=5, home_away=True)
...
def final_round_button(request: HttpRequest) -> HttpResponse:
    ...
    if request.method == 'GET':
        _service().perform_draw('final', pools_count=5, teams_per_pool=5, home_away=True)
...
def team_list_filter(text: str) -> List[Dict[str, Any]]:
    ...
    return _service().filter_teams(text)