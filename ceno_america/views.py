...
def final_round(request: HttpRequest) -> HttpResponse:
    ...
    context = _service().get_round_context('final', GROUP_KEYS[0:3], team_size=5, group_range=GROUP_RANGE)
...
def final_round_button(request: HttpRequest) -> HttpResponse:
    ...
    if request.method == 'GET':
        _service().perform_draw('final', pools_count=5, teams_per_pool=3, home_away=True)
